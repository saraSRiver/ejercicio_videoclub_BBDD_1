...
--Indica el nombre, ciudad, y teléfono de todos los miembros del videoclub que hayan alquilado películas 
--que hayan sido nominadas a más de 150 premios y ganaran menos de 50
SELECT me.MEMBER_NAME, me.MEMBER_PHONE, me.MEMBER_TOWN, m.MOVIE_NAME,a.AWARD_NOMINATION,a.AWARD_WIN  FROM MEMBERS me
INNER JOIN MEMBERS_MOVIE_RENTAL mr ON mr.MEMBER_ID  = me.MEMBER_ID
INNER JOIN MOVIES m ON m.MOVIE_ID = mr.MOVIE_ID
INNER JOIN AWARDS a ON a.MOVIE_ID = m.MOVIE_ID
GROUP BY  me.MEMBER_NAME,me.MEMBER_PHONE, me.MEMBER_TOWN, m.MOVIE_NAME,a.AWARD_NOMINATION,a.AWARD_WIN
HAVING a.AWARD_NOMINATION > 150 and a.AWARD_WIN < 50


--Indica cual es el nombre del director del que más películas se han alquilado 
SELECT count(mr.MEMBER_MOVIE_RENTAL_ID) AS num_alquiler, m.MOVIE_NAME, d.DIRECTOR_NAME 
from MEMBERS_MOVIE_RENTAL mr
INNER JOIN MOVIES m ON m.MOVIE_ID = mr.MOVIE_ID
INNER JOIN DIRECTORS d ON m.DIRECTOR_ID = d.DIRECTOR_ID 
...
--Indica cuál es la nacionalidad favorita de cada uno de los estudios en la producción de las películas
 SELECT s.STUDIO_NAME , n.NATIONALITY_NAME , COUNT(*) AS TotalMovies
FROM STUDIOS s
...
ORDER BY s.STUDIO_NAME, TotalMovies DESC;
